...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const tip1 = document.getElementById('tip1') as HTMLParagraphElement;
    const tip2 = document.getElementById('tip2') as HTMLParagraphElement;
    const btnC = document.getElementById('btn-color') as HTMLButtonElement;
    const label1 = document.getElementById('about-author') as HTMLLabelElement;
    const label2 = document.getElementById('about-version') as HTMLLabelElement;
    const p1 = document.getElementById('about-explain') as HTMLParagraphElement;
    const a1 = document.getElementById('edge_page') as HTMLAnchorElement;
    const th1 = document.getElementById('th1') as HTMLTableHeaderCellElement;
    const th2 = document.getElementById('th2') as HTMLTableHeaderCellElement;
    const th3 = document.getElementById('th3') as HTMLTableHeaderCellElement;
...
document.getElementById('btn-n')?.addEventListener('click', function () {
    ...
    request.onupgradeneeded = function (event) {
        // 在数据库升级时创建对象存储
        db = (event.target as any).result as IDBDatabase;db = (event.target as any).result as IDBDatabase;
    ...
    request.onerror = function (event) {
        // 打开或创建数据库时发生错误
        const errorEvent = event as Event & { target: IDBOpenDBRequest & { error?: DOMException } };
        console.error('ero:', errorEvent.target.error?.message);
    };
